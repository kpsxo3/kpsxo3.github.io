...
const randomizedTombs = shuffleArray(tombItems)

const tombZonesMap = []
for (let i = 0; i < tombZonesData.length; i += 70) {
  tombZonesMap.push(tombZonesData.slice(i, 70 + i))
}

const tombZones = []
let tombIndex = 0

tombZonesMap.forEach((row, i) => {
  row.forEach((symbol, j) => {
    if (symbol === 1281) {
      const itemData = randomizedTombs[tombIndex] || { title: '', image: null }
...
const movables = [background, ...boundaries, foreground, ...battleZones, ...treasureZones, ...tombZones]
...
// TOMB
  for (let i = 0; i < tombZones.length; i++) {
    const tombZone = tombZones[i]
    if (
      mouseX >= tombZone.position.x &&
      mouseX <= tombZone.position.x + Boundary.width &&
      mouseY >= tombZone.position.y &&
      mouseY <= tombZone.position.y + Boundary.height
    ) {
      if (tombZone.image) {
        openBioResult(tombZone.image, tombZone.title)
      }
      break
    }
  }
...
function animate() {
	...
	tombZones.forEach((tombZone) => tombZone.draw())
